...
## Parameter Defaults & Safety

When adding configurable parameters (especially for resource-intensive operations):

### Quality/Resolution Parameters
- Always set conservative defaults (e.g., "480p" not "1080p", "worst" not "best")
- Document why the default is conservative
- Include warning comments if changing defaults could cause large resource consumption
- For streaming/download operations: prefer lowest quality by default to prevent accidental 15-25GB downloads

### Testing Parameters
- Set defaults that work well for CI/CD (fast, deterministic)
- Allow overrides via kwargs or env vars for real-world testing
- Never hardcode real URLs in code; use mocks in tests, real URLs in manual docs only
- Example: `quality: str = "480p"` rather than `quality: str = "best"`

### Resource-Intensive Operations
- Always have explicit limits or guards
- Add comments explaining why the default exists
- Include examples of safe vs. unsafe parameter combinations
- Test with minimal data first (low resolution, small datasets, short durations)
...
When creating temporary files for manual testing or verification:

1. **Do NOT commit manual test files** — Examples:
   - Manual test scripts (e.g., `test_manual_download.py`, `verify_*.py`)
   - Generated output directories (e.g., `manual_test_output/`, `test_downloads/`)
   - Real API responses or downloaded files (e.g., large mp4s, json dumps from live APIs)

2. **Always clean up before finishing**:
   - Delete all temporary test scripts used for manual verification
   - Delete all generated output directories
   - Delete downloaded or cached files
   - Use `git status` as final check — should only show source changes

3. **Update .gitignore if pattern will recur**:
   - If you add a new tool that generates artifacts (e.g., `manual_test_output/`)
   - Add to `.gitignore` in same commit as code changes
   - Verify `git status` shows nothing in that directory

4. **Test artifacts to keep** (these SHOULD be committed):
   - Small, deterministic test data in `tests/media/` or `tests/fixtures/`
   - Synthetic/generated test videos (under 10MB each, pre-generated by scripts)
   - Mock data JSON files used by unit tests

### Verification Before Finishing

Before declaring a task complete, verify:

- [ ] Run `pytest --cov=backend` — passes with ≥80% coverage per file
- [ ] `git status` shows ONLY source files (no artifacts, no large downloads)
- [ ] No untracked directories left (manual_test_output/, downloads/, cache/, etc.)
- [ ] No large files in git diff (*.mp4, *.zip, *.tar.gz, ≥5MB)
- [ ] `.gitignore` updated if new artifact patterns will recur
- [ ] Documentation (README.md / docs/) updated if user-facing behavior changed
...
- How to verify: `pytest tests/backend/test_vod_download.py -v`

**How to Run**
- Exact command used to verify (e.g., `pytest --cov=backend`)
- Pass/fail summary
- Coverage impact

**Notes**
- Cleanup status (what was removed, if manual testing was done)
- Known limitations or deferred work
- Any risks or edge cases
- Dependencies added (if any)

**Artifact Verification** (for work involving manual testing/downloads)
- [ ] Temporary files deleted (test scripts, manual output dirs)
- [ ] `git status` is clean (only source changes shown)
- [ ] No large files staged or untracked
- [ ] `.gitignore` updated for any new artifact patterns
